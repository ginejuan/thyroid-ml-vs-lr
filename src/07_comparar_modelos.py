"""
07_comparar_modelos.py
----------------------
Reúne las métricas y predicciones de los 5 modelos, genera la tabla
comparativa, las curvas ROC, los gráficos de calibración y la curva de
decisión (net benefit) — todo sobre las mismas predicciones CV 10×5.
"""
...
print("\n=== TABLA COMPARATIVA ===\n")
print(tbl[["model","AUC (IC95%)","brier","cal_slope","cal_intercept",
           "sens_youden","spec_youden","sens_paper","spec_paper"]]
      .to_string(index=False, float_format=lambda x: f"{x:.3f}"))

tbl.to_csv(RESDIR / "comparacion_modelos.csv", index=False)
print(f"\nGuardado: {RESDIR/'comparacion_modelos.csv'}")
...
for key, name in MODELS:
    ...
    ax.plot(fpr, tpr, label=f"{name} (AUC={auc:.3f})", color=COLORS[key], lw=1.7)
ax.plot([0, 1], [0, 1], "--", color="gray", lw=0.8)
ax.set_xlabel("1 − especificidad")
ax.set_ylabel("Sensibilidad")
ax.set_title("Curvas ROC (CV 10×5)")
ax.legend(loc="lower right", fontsize=9)
ax.set_aspect("equal")
plt.tight_layout()
plt.savefig(FIGDIR / "roc_curves.png", dpi=150)
plt.close()
print(f"Guardado: {FIGDIR/'roc_curves.png'}")

# 4. Calibración (loess no disponible en sklearn; usamos bins quantile) -
fig, ax = plt.subplots(figsize=(6, 6))
...
ax.set_title("Calibración (deciles cuantil, CV 10×5)")
ax.set_xlim(0, 1); ax.set_ylim(0, 1)
ax.legend(loc="upper left", fontsize=9)
ax.set_aspect("equal")
plt.tight_layout()
plt.savefig(FIGDIR / "calibration_curves.png", dpi=150)
plt.close()
print(f"Guardado: {FIGDIR/'calibration_curves.png'}")

# 5. Curva de decisión (net benefit) ------------------------------------
...
ax.set_title("Curva de decisión (CV 10×5)")
ax.set_ylim(-0.02, 0.25)
ax.legend(loc="upper right", fontsize=9)
plt.tight_layout()
plt.savefig(FIGDIR / "decision_curves.png", dpi=150)
plt.close()
print(f"Guardado: {FIGDIR/'decision_curves.png'}")
